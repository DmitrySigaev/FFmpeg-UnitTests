...
FF_ENABLE_DEPRECATION_WARNINGS
    ...
    if (ret) {
        if (ctx->flags & AV_CODEC_FLAG_PSNR) {
            u_stat_t stat = (u_stat_t){ pic_out.prop.f_ssim, pic_out.prop.f_psnr_avg, pic_out.prop.f_psnr[0], pic_out.prop.f_psnr[1], pic_out.prop.f_psnr[2], pic_out.prop.f_crf_avg };
            ff_side_data_set_encoder_stats(pkt, (pic_out.i_qpplus1 - 1) * FF_QP2LAMBDA, (int64_t*)&stat, (sizeof(u_stat_t) / sizeof(int64_t)), pict_type);
        }
        else {
